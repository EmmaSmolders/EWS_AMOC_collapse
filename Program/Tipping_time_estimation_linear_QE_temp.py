...
data_zero = loadmat(directory + 'time_at_zero_all_SAMBA_TEMP_QE_1500_1700_window_70_change_1580_1645_all_CP_pertimeseries_NOSELECTIONBIAS.mat')
time_zero = data_zero['time_at_zero_all_start']
time_zero = time_zero[:,:,:,0:len(lon_grid)]
time_zero = ma.masked_invalid(time_zero)

fh_r = loadmat(directory + 'r_value_all_SAMBA_TEMP_QE_1500_1700_window_70_change_1580_1645_all_CP_pertimeseries_NOSELECTIONBIAS.mat')
r_value = fh_r['r_value_all_start']
r_value = r_value[:,:,:,0:len(lon_grid)]
r_value = ma.masked_invalid(r_value)

fh_trend = loadmat(directory + 'trend_all_SAMBA_TEMP_QE_1500_1700_window_70_change_1580_1645_all_CP_pertimeseries_NOSELECTIONBIAS.mat')
trend = fh_trend['trend_all_start']
trend = trend[:,:,:,0:len(lon_grid)]
trend = ma.masked_invalid(trend)

fh_base = loadmat(directory + 'base_all_SAMBA_TEMP_QE_1500_1700_window_70_change_1580_1645_all_CP_pertimeseries_NOSELECTIONBIAS.mat')
...
plt.title('Estimated tipping times (temperature) for all fits (R>0)')
...
axs.set_title('R values (temperature, window 70 years)')
...
for i in range(9):
    ...
    data_point = lambda_temp[:, depth_i, lon_i]
    time_point = time
...
plt.tight_layout()
#plt.suptitle(f'Restoring rate temperature SAMBA, end year {end_year}, MinThreshold {minthreshold}')
plt.savefig(directory_figures + 'LAMBDA_QE_fit_to_zero_SAMBA_TEMP_cesm_w70_minthreshold1_r_07_9worstfits_1758.pdf')
plt.show()
...
ax[1].set_xlim(1700, 1955)
ax[1].set_ylim(0,0.05)

plt.tight_layout()
#plt.suptitle('Temperature along SAMBA (quasi-equilibrium)', y=1.02, fontsize=20)
plt.savefig(directory_figures + 'mean_median_CI_temperature_samba_QE_range_CP_restriction_allgridpoints_areaweighted_window70_1500_1700_ALLCP_R07.pdf', bbox_inches='tight')
...
ax[0].fill_between(end_year_change[:], ci_lower_bin_center_positive, ci_upper_bin_center_positive, color='blue', alpha=0.2, label='10-90% CI')
ax[0].plot(end_year_change[:], ci_lower_bin_center_positive, color='blue', linewidth = 2, alpha=0.8)
ax[0].plot(end_year_change[:], ci_upper_bin_center_positive, color='blue', linewidth = 2, alpha=0.8)
ax[0].axhline(y=1758, color='red', linestyle='-', label='CESM tipping point')
#ax[0].fill_between(end_year_change_1500[10::], 1741, 1775, color='red', alpha=0.4)
...
ax[1].stairs(final_hist_positive[-1,:], bins_positive, fill=True, color='blue', alpha=0.8)
ax[1].grid('True')
...
plt.tight_layout()
#plt.suptitle('Temperature along SAMBA (quasi-equilibrium)', y=1.02, fontsize=20)
plt.savefig(directory_figures + 'mean_median_CI_temperature_samba_QE_range_CP_restriction_allgridpoints_areaweighted_window70_1500_1700_ALLCP_R0.pdf', bbox_inches='tight')
...
plt.colorbar()
plt.contourf(lon, depth, sig_lambda_temp_neg_3, colors='none', linewidth=1.0, hatches='xx')
...
plt.savefig(directory_figures + 'Median_percentage_tipping_time_ALL_positive_R07_temperature_samba_QE_range_CP_restriction_allgridpoints_areaweighted_window70_1500_1700_ALLCP.pdf', bbox_inches='tight')
