...
lambda_salt       = -lambda_salt #Restoring rate is negative in Boers but should be negative
...
data = loadmat(directory + 'changepoint_time_all_SAMBA_SALT_QE_1500_1700_window_70_change_1580_1680_all_CP_pertimeseries_NOSELECTIONBIAS.mat')
...
plt.xlabel('Model years')
plt.ylabel('Frequency')
plt.title('Estimated tipping times (salinity) for all negative fits')
...
plt.xlabel('Model years')
plt.ylabel('Frequency')
plt.title('Estimated tipping times (salinity) for all R < 0.7 fits')
...
plt.xlabel('Model years')
plt.ylabel('Frequency')
plt.title('Estimated changepoint times (salinity) for all fits')
...
plt.xlabel('Model years')
plt.ylabel('Frequency')
plt.title('Estimated slopes (salinity) for all fits')
...
plt.xlabel('Model years')
plt.ylabel('Frequency')
plt.title('Estimated intercepts (salinity) for all fits')
...
for i in range(9):
    ...
    data_point = lambda_salt[:, depth_i, lon_i]**2
    time_point = time
...
#plt.savefig(directory_figures + 'LAMBDA_transient_fit_to_zero_SAMBA_SALT_RCP85_w10_minthreshold1_r_07.pdf')
plt.show()
...
plt.tight_layout()
#plt.suptitle(f'Restoring rate temperature SAMBA, end year {end_year}, MinThreshold {minthreshold}')
plt.savefig(directory_figures + 'LAMBDA_QE_fit_to_zero_SAMBA_SALT_cesm_w70_minthreshold1_r_07_9worstfits_1758_quadratic.pdf')
plt.show()
...
plt.savefig(directory_figures + 'mean_median_CI_salinity_samba_QE_range_CP_restriction_allgridpoints_areaweighted_window70_1500_1700_ALLCP_R07_quadratic.pdf', bbox_inches='tight')
...
ax[0].fill_between(end_year_change[:], ci_lower_bin_center_negative, ci_upper_bin_center_negative, color='green', alpha=0.2, label='10-90% CI')
ax[0].plot(end_year_change[:], ci_lower_bin_center_negative, color='green', linewidth = 2, alpha=0.8)
ax[0].plot(end_year_change[:], ci_upper_bin_center_negative, color='green', linewidth = 2, alpha=0.8)
ax[0].axhline(y=1758, color='red', linestyle='-', label='CESM tipping point')
#ax[0].fill_between(end_year_change_1500[10::], 1741, 1775, color='red', alpha=0.4)
...
ax[1].stairs(final_hist_negative[-1,:], bins_negative, fill=True, color='green', alpha=0.8)
ax[1].grid('True')
...
plt.tight_layout()
#plt.suptitle('Temperature along SAMBA (quasi-equilibrium)', y=1.02, fontsize=20)
plt.savefig(directory_figures + 'mean_median_CI_salinity_samba_QE_range_CP_restriction_allgridpoints_areaweighted_window70_1500_1700_ALLCP_R0_quadratic.pdf', bbox_inches='tight')
...
#%% Determine mean and 90% for final histogram over different ranges of end years 
...
#plt.savefig(directory_figures + 'Median_percentage_tipping_time_ALL_negative_R07_salinity_samba_QE_range_CP_restriction_allgridpoints_areaweighted_window70_1500_1700_ALLCP.pdf', bbox_inches='tight')
...
plt.savefig(directory_figures + 'Median_percentage_tipping_time_ALL_negative_R07_salinity_samba_QE_range_CP_restriction_allgridpoints_areaweighted_window70_1500_1700_ALLCP_quadratic.pdf', bbox_inches='tight')
