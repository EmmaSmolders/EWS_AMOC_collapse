...
def ReadinData(filename, lat_index, depth_min_index, depth_max_index):
	...
	lon_u 		= fh.variables['ULONG'][lat_index]						#Longitude
	...
	dx_u		= fh.variables['DXU'][lat_index]  / 100.0		                        #Zonal grid cell length (m)
	depth_u 	= fh.variables['HU'][lat_index] / 100.0						#Depth at u-grid (m)
	v_vel 		= fh.variables['VVEL'][0, depth_min_index:depth_max_index, lat_index] / 100.0	#Meridional velocity (m/s)


	#Get the t-grid
	lon_t 		= fh.variables['TLONG'][lat_index:lat_index+2]							#Longitude
	lat_t 		= fh.variables['TLAT'][lat_index:lat_index+2]					#Latitude 
	dx_t		= fh.variables['DXT'][lat_index:lat_index+2]  / 100.0		                   #Zonal grid cell length (m)
	depth_t 	= fh.variables['HT'][lat_index:lat_index+2] / 100.0					#Depth at t-grid (m)
	salt		= fh.variables['SALT'][0, depth_min_index:depth_max_index, lat_index:lat_index+2] 	#Salinity (g / kg)

	fh.close()

	#Convert the grid to -180 to 180 u-grid
	lon, depth_u	= ConverterField1D(lon_u, depth_u)
	lon_u, v_vel	= ConverterField2D(lon_u, v_vel)

	#Convert the grid to -180 to 180 t-grid
	lon, depth_t[0]	= ConverterField1D(lon_t[0], depth_t[0])
	lon, depth_t[1]	= ConverterField1D(lon_t[1], depth_t[1])
	lon_t, salt	= ConverterField3D(lon_t, salt)

	#Now seelct only the South Atlantic boundary
	lon_1, lon_2	= 105, 180

	lon_u	= lon_u[lon_1:lon_2]
	lat_u	= lat_u[lon_1:lon_2]
	lon_t	= lon_t[:, lon_1:lon_2]
	lat_t	= lat_t[:, lon_1:lon_2]
	depth_u	= depth_u[lon_1:lon_2]
	depth_t	= depth_t[:, lon_1:lon_2]
	dx_u	= dx_u[lon_1:lon_2]
	dx_t	= dx_t[:, lon_1:lon_2]
	v_vel	= v_vel[:, lon_1:lon_2]
	salt	= salt[:, :, lon_1:lon_2]

	return lon_u, lat_u, lon_t[0], lat_t, depth, layer, depth_u, depth_t, dx_u, dx_t, v_vel, salt

def ConverterField1D(lon, field):
...
lat_u 		= fh.variables['ULAT'][:, 10]	#Latitude  
...
lat_index		= (np.abs(lat_u - lat_MOV)).argmin()

#-----------------------------------------------------------------------------------------
#Determine the section length per depth layer
lon_u, lat_u, lon_t, lat_t, depth, layer, depth_u, depth_t, grid_x_u, grid_x_t, v_vel, salt 	= ReadinData(files[0], lat_index, depth_min_index, depth_max_index)
...
for year_i in range(len(time_year)):
	...
	v_vel	=    ma.masked_all((12, len(depth), 2, len(lon_u)))
	salt	=    ma.masked_all((12, len(depth), 2, len(lon_t)))

	for file_i in range(len(files_month)):
		lon_u, lat_u, lon_t, lat_t, depth, layer, depth_u, depth_t, grid_x_u, grid_x_t, v_vel_month, salt_month = ReadinData(files_month[file_i], lat_index, depth_min_index, depth_max_index)
	...
	month_days	= ma.masked_array(month_days, salt[:, 0, 0, 40].mask)
...
fh = netcdf.Dataset(directory+'Ocean/FOV_34S_year_'+str(year_start)+'_'+str(year_end)+'_branch_1650.nc', 'w')
