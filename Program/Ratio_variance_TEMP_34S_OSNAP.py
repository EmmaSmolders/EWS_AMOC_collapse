...
for depth_i in range(len(depth)):
	...
	for lon_i in range(len(lon)):
		...
		#Variance
		var_temp1[depth_i, lon_i] = np.var(TrendRemover(time1, temp1[:, depth_i, lon_i]))
		var_temp2[depth_i, lon_i] = np.var(TrendRemover(time2, temp2[:, depth_i, lon_i]))

		#Levene's test to test statistical difference between E1 and E2
		stat[depth_i, lon_i], p[depth_i, lon_i] = stats.levene(TrendRemover(time1, temp1[:, depth_i, lon_i]), TrendRemover(time2, temp2[:, depth_i, lon_i]))
	...
	fh = netcdf.Dataset(directory+'Atlantic_single_estimator_variance_TEMP_month_'+str(month_start)+'-'+str(month_end)+'_'+str(transect)+'_branch_'+str(branch2)+'_'+str(branch1)+'.nc', 'w')
